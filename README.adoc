...
==== '[output cu un frame pe video, făcut doar pentru sunet]'

- `-loop 1 -i cover.jpg -i "sunet.mp3" -c:v libx264 -crf 18 -tune
  stillimage -c:a libfdk_aac -vbr 4 -pix_fmt yuv420p -shortest output.mp4` unde loop 1 este loop pentru primul input, în cazul de față poza. `-shortest` de obice oprește encoding-ul la cel mai scurt input, care în combinație cu `-loop` a devenit audio-ul, pentru că imaginea e infinit loop.
